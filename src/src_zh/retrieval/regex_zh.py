...
logging.basicConfig(
    ...
    level=logging.INFO,
    filemode="w",
    filename=os.path.join("src_zh", "retrieval_legacy", "regex_zh.log"))
...
def extract_ai_passages(input_dir: str, country:str):
    """
    ...
    logging.info(f"Processed {len(reports)} reports with {total_chunks} chunks for {country}")
    logging.info(f"Found AI related mentions in {len(reports) - no_matches} reports")
    logging.info(f"Found {matched_chunks} chunks overall")
